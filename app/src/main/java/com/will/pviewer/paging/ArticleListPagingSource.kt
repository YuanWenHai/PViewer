...
class ArticleListPagingSource(private val apiService: ApiService,private val series: String): PagingSource<Int,ArticleWithPictures>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ArticleWithPictures> {
        ...
        Log.d(LOG_TAG,"on pagingSource load -> series: $series   pageCount:$pageCount   pageSize:$pageSize paramsKey: ${params.key}")
        if(response.code() != 200){
            val errorMsg = "network error on getArticleList() request, response code is: ${response.code()} error response body string is: ${response.errorBody()?.string()}"
